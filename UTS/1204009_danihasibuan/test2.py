...
def process_payment(payment_queue, amount):
    ...
    time.sleep(1)
    end_time = time.time()
    print(f"Memproses pembayaran sejumlah {amount} dalam waktu {end_time - start_time:.2f} detik")
...
def rent_item(rental_queue, item, price):
    ...
    time.sleep(2)
    end_time = time.time()
    print(f"Barang {item} berhasil disewakan dengan harga {price} dalam waktu {end_time - start_time:.2f} detik")
